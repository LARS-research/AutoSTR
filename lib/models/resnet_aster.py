...
class Tiny_ResNet_ASTER(nn.Module):
  """For aster or crnn"""

  def __init__(self, with_lstm=False, n_group=1, **kwargs):
    super(Tiny_ResNet_ASTER, self).__init__()
    self.with_lstm = with_lstm
    self.n_group = n_group
    ...
    self.inplanes = 32
    self.layer1 = self._make_layer(32,  2, [2, 2]) # [16, 50]
    self.layer2 = self._make_layer(64,  2, [2, 2]) # [8, 25]
    self.layer3 = self._make_layer(128, 3, [2, 1]) # [4, 25]
    self.layer4 = self._make_layer(256, 3, [2, 1]) # [2, 25]
    self.layer5 = self._make_layer(512, 1, [2, 1]) # [1, 25]
